...
void arduinoSerialMonitorVisual(char symbol, int data )
{
  ...
  if (pulseCount > 0)
  {
    pulseWidth = NUM_LEDS / 8;
    pulseCount--;
  }
  if (pulseWidth > 0)
    pulseWidth--;
  animateStrip(pulseWidth);
  return;

  int filtered = historyFilter(data);
  if (historyFilter(data) > 0)
  {
    //    Serial.println("boom");
    animateStrip(filtered);
    graph(data, '+');
  }
  else
  {
